...
const signAndRespondToTask = async (taskIndex: number, taskCreatedBlock: number, taskName: string) => {
    console.log(`Evaluating MMLU subset: ${taskName}`);
    
    try {
        const response = await axios.get(`http://192.168.1.250:5000/evaluate?subset=${taskName}`);
        const evaluationResult = response.data;
        const accuracy = Math.floor(evaluationResult.accuracy * 10000); // Convert to basis points (e.g., 0.7523 -> 7523)
        ...
        await tx.wait();
        console.log(`Responded to task with accuracy: ${accuracy / 100}%`);
    } catch (error) {
        console.error('Error evaluating MMLU subset:', error);
    }
};
